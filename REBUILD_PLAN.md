...
## 0. Current status — READ THIS FIRST (updated 2026-05-29)

This block is the resume point. Whoever picks this up next: start here, then continue from **NEXT**.

- 🎨 **Windows Update view — feedback & glanceability overhaul (built 2026-05-29, feature branch, 122 tests green, 0 warnings, launch-verified locally).** UX pass after a research-backed review (plan: `.claude/plans/ultrathink-look-over-this-hashed-fox.md`); architecture kept, feedback made glanceable. All four clusters shipped:
  - **One source of truth:** computed `Computer.PatchState` (Idle/Scanning/Available/Downloading/Installing/RebootPending/Done/Error) from `UpdatePhase` + `RebootRequired`. Resolves the two lying cases: `PendingReboot + !RebootRequired ⇒ Done` (green back-online), `Done/Available/Idle + RebootRequired ⇒ RebootPending` (amber). Drives chip, message colour, fleet counts (14 tests).
  - **Cluster 1 — glanceable:** new **Status chip** column (colored pill, after Ping) via `PhaseChipConverter` (frozen brushes; grey/blue/steel/amber/green/red); **update message colored** by state; **Command-messages column removed from the UpdateGrid** (still in Machines mode).
  - **Cluster 2 — fleet + progress (Both):** VM `FleetSummary`/counts/`IsPatchOperationActive`/`FleetProgress` riding the existing `CollectionChanged`/`PropertyChanged` hooks (no new loops); **bottom bar** appends "Updates: N working · …" beside Online; **band above the grid** (overall bar) only during an active op. Per-row progress: blank when idle, marquee while scanning, % while installing, ✓/✕ at end.
  - **Cluster 3 — toast + flash:** `WorkspaceViewModel.OperationCompleted` event (once per sweep, not per row) → tray balloon via the existing `TaskbarIcon`, **suppressed when window focused**; green/red row-flash storyboard on state change (RowStyle `BasedOn` the WPF-UI default to keep selection visuals).
  - **Cluster 4 — unified entry points:** toolbar Scan/Install relabel **"Scan/Install selected (N)"** vs **"…all"** (`ScanTarget`/`InstallTarget` commands route selected-or-all); **first-run hint** band in update mode until a scan runs.
  - **Bug found + fixed during verify:** the fleet band's `ProgressBar.Value="{Binding FleetProgress}"` bound a read-only prop two-way → `InvalidOperationException` at load → the global catch swallowed it and **blanked the whole grid**. Fixed with `Mode=OneWay`. Lesson: read-only props bound to `ProgressBar.Value` must be `Mode=OneWay`. Relaunched clean (no log errors), grid restored, user confirmed "looks so much better."
  - Files: `Computer.cs`, `HostPatchStatus.cs` (PatchState enum), `PhaseChipConverter.cs` (new), `WorkspaceView.xaml`, `MainWindow.xaml(.cs)`, `WorkspaceViewModel.cs`, new `ComputerPatchStateTests`. **NEXT:** user testing the fresh zip; possible color/label tweaks next session. Then live-verify chips/band/toast against a real install→reboot→back-online cycle.


- 🛡 **Servicing-collision hardening (built 2026-05-29, working tree, 108 tests green, 0 warnings; boot-busy guard verified live on the dev box).** Defensive pass after a Server 2019 target hit a boot-time servicing rollback (STATUS_SHARING_VIOLATION at CSI ResolvePendingTransactions) — investigation showed **Vivre did not cause it** (agent finished rc=0, task deleted ~9h before), but it motivated guards so Vivre can never collide with the OS reboot + offline-servicing pass. Plan: `.claude/plans/ultrathink-look-over-this-hashed-fox.md`. All changes isolated to the update lane; normal path unchanged. Six items:
  1. **Agent boot-busy guard** — new pure `BootServicingState.Evaluate(...)` (linked into the test project) + agent-only `BootBusyGuard` I/O shim reading CBS `RebootInProgress`/`RebootPending`/`PackagesPending`, `WinSxS\pending.xml`, `PendingFileRenameOperations`, WU `RebootRequired`. `Program.Main` checks it **before** any WUA/DISM and, if busy, writes a `PendingReboot` "Deferred — …" line and exits 0. **Verified live:** the dev box had `PendingFileRenameOperations` queued → agent deferred cleanly without touching the servicing stack.
  2. **`-StartWhenAvailable` removed** from `BuildBootstrapScript` task settings (inert on run-now since there's no trigger; on `ScheduleAt` it was the hazard — a missed one-time trigger could re-fire at boot). Asserted absent by a test.
  3. **Per-host serialization in `PatchService`** — a `ConcurrentDictionary` claims the host for install/uninstall/**Installed-scope scan** (the CBS/DISM ops); a concurrent same-host request is **skipped** with an Idle "already in progress" status. This is the authoritative guard the singleton needs — it catches the **cross-tab same-host** case the per-row `IsPatching` (per-tab `Computer`) can't. Applicable scan is exempt (read-only). Plus a servicing-busy check in `BuildScanScript` skips `Get-WindowsPackage` (falls back to WUA-only removability) when a reboot/`pending.xml` is detected.
  4 + 6. **COM release + reboot ordering** — `RunInstall`/`RunUninstall`/`Summarize` now **return `rebootNeeded`** and no longer call `shutdown`; `Main` releases the WUA RCWs (`GC.Collect`+`WaitForPendingFinalizers`) then `shutdown /r /t 20` and **exits immediately** (no `Thread.Sleep` holding the process/COM open). `ProgressWriter` already flushes per write, so the terminal line is on disk first.
  5. **Cleanup race** — the bootstrap `finally` (and `BuildSafetyCleanupScript`) now **wait for the task to leave Running** (≤10s) before `Remove-Item` the EXE (a running EXE is file-locked); with the agent exiting immediately after scheduling `/t 20`, the task frees within ~1s, well inside the 20s reboot delay.
  - `BuildBootstrapScript` made `internal` + `[InternalsVisibleTo("Vivre.Core.Tests")]` on Core. New tests: `BootServicingStateTests`, `BootstrapScriptTests` (no `-StartWhenAvailable`), `PatchServiceTests` (per-host serialization via a blocking fake host). Build 0 warnings; **108 tests green** (was 95); bootstrap + Installed-scan scripts parse clean under PS 5.1. Fresh zip (agent 31 KB) ready.
  - **NEXT (live):** on a real target — confirm a normal install still works end-to-end (it doesn't trip the guard on a clean box), the per-host skip fires if you launch the same host twice (e.g. two tabs), and a RebootAndWait reboots ~20s after completion with the temp EXE cleaned up (no orphaned `Vivre_WUA_*`).


- 🧰 **Update-lane UX + concurrency + WUA-then-DISM uninstall (built 2026-05-29, working tree, 95 tests green, 0 warnings, PENDING LIVE-VERIFY).** Addressed a batch of user-reported issues:
  - **Scan corrupted an in-flight install** (right-click *Updates ▸ Scan/Install selected* weren't busy-gated, so a concurrent scan stomped the installing row + the single shared CTS). Fix: `Computer.IsPatching` (set for the whole install/uninstall) — a scan **skips** a patching row, a second install/uninstall won't start on one already patching; and the VM now tracks **all** active operations in `_activeCts` (ref-counted `IsBusy`, shared `_patchThrottle`) so **Stop cancels every running op** and you can **add + scan a new machine while another installs** without corruption. (`RunSweepAsync`/`RunPatchSweepAsync` use `BeginOperation`/`EndOperation`; `_cts` field removed.)
  - **Uninstall checklist all-checked → now defaults UNCHECKED** in Installed scope (removal is opt-in); install scope still defaults checked. **"All" skips non-removable** rows. **"Uninstall checked" button** now bound to `CanUninstallChecked` (was a `Click` handler ignoring the gate) and **re-evaluates live as boxes toggle** (the VM subscribes to the focused checklist's `IsSelected` changes via `RetrackChecklist`); disabled before a scan / when nothing removable is ticked. `CanInstallChecked`/`CanUninstallChecked` are now public properties (bound for enable-state + used as `RelayCommand` CanExecute). Side-panel Scan relabeled **"Scan this machine"**; toolbar **"Scan all"** (the all-rows sweep, which exists in MainWindow — the "scan only did one" confusion was the side-panel per-machine button).
  - **Uninstall engine = WUA then DISM (future-proof; no deprecated wusa).** `Computer`/`SoftwareUpdate.IsUninstallable` now means **removable by *some* engine**: the Installed scan unions WUA `IsUninstallable` with the DISM-removable `Package_for_KB` set (`Get-WindowsPackage -Online`, read-only over WinRM, graceful fallback to WUA-only if it can't enumerate) — so greyed = neither engine can remove. The **agent's `RunUninstall`** iterates the ticked KBs: WUA `BeginUninstall` (live %) when WUA-uninstallable, else **`dism.exe /online /Remove-Package`** resolved from the KB via `dism /online /get-packages` (`DismHelper.cs`), reporting per-KB results incl. "no supported uninstall path." Reboot (DISM 3010 / WUA RebootRequired) surfaces as PendingReboot.
  - **Verified offline:** full build 0 warnings, 95 tests green, both scan scripts (Applicable + Installed) parse clean under PS 5.1. **NOT yet live-verified:** the DISM paths (`dism get-packages`/`Remove-Package` need elevation — agent runs as SYSTEM so they work there; couldn't exercise from the non-elevated dev shell) and `Get-WindowsPackage` over WinRM. Worst case both degrade gracefully (WUA-only removability; "no supported uninstall path" reporting).
  - **Install live-verified working on a real target (2026-05-29)** — download/install progress shown (the message % is the current step; the bar is overall download[0-50%]+install[50-100%], so "Installing 1 of 1 — 20%" reads 60% on the bar — by design). **Follow-up UX fixes (post-test, in tree):**
    - While the focused machine is installing/uninstalling (`Computer.IsPatching`) the side-panel checklist (DataGrid + All/None) and its Install/Uninstall buttons now **lock** (VM `IsFocusedPatching` + `!c.IsPatching` in the Can* gates, observing the focused computer's `IsPatching`); the right-click *Updates ▸ Scan/Install selected* items are **disabled** when every selected row is already patching — they no longer stay clickable after launching an install.
    - **Reboot lifecycle reporting (A2 partial):** the monitor writes the **Reboot message** column on online↔offline transitions — `Computer.WentOfflineAt` records the down moment, return shows "Back online HH:mm (down 3m) — reboot still pending". Caveat: monitor pauses while `IsBusy` / 20s ticks, so the offline window can be missed.
    - **Reboot-complete made robust (2026-05-29, post-test fix):** the online↔offline transition alone was unreliable for the "it's back" message (missed down windows → silence; stale "Installed 1 update(s), reboot required" + a restart script's "Restart scheduled…" just sat there). Fix: drive the reliable signal off the **`RebootRequired` true→false transition** in `ProbeRebootPendingAsync` (the probe that turns the dot green, so it always runs) → writes "Reboot complete — back online HH:mm" and **strips the now-stale ", reboot required"** from `UpdateMessage`. "Back online" also now fires on any `previous==false`→online return even without a captured `WentOfflineAt`. (Command-result column left as-is — generic script output; use Clear Results.)
  - **NEXT (live-verify):** (1) add a machine mid-install → it scans without disturbing the installer; Stop cancels all. (2) Installed-scope scan: removable rows checkable, truly-non-removable greyed w/ tooltip, default unchecked, All skips greyed, Uninstall button disabled until a removable KB is ticked. (3) Uninstall a WUA-uninstallable KB (live %) and a DISM-only KB (e.g. a standalone `Package_for_KB`), confirm removal + per-KB results + reboot prompt.


- 🧩 **Compiled WUA agent replaces the PowerShell install/uninstall worker — true BatchPatch-style live progress (built 2026-05-28, working tree, 95 tests green, build 0 warnings, PENDING LIVE-VERIFY).** Two problems drove this: (a) **Stop froze the UI ~2 min** when a target rebooted mid-scan, and (b) the download forever fell to `Downloading … (sync mode — no live progress)`.
  - **(a) Stop-freeze — FIXED in `PSRunspaceHost`.** Root cause: `Stop()` is a UI-thread `[RelayCommand]` calling `_cts.Cancel()`, and `CancellationTokenSource.Cancel()` runs registered callbacks **synchronously inline on the caller (UI) thread**. The registered callback was `ps.Stop()`, which **blocks until the pipeline actually stops** — for a remote pipeline to a rebooting host that's the full WSMan timeout, ×N in-flight hosts = the multi-minute freeze. Fix: the callback now calls **`ps.BeginStop(null, null)`** (initiates the stop, returns instantly); the awaited `InvokeAsync` still unwinds with `PipelineStoppedException`, the runspace disposes, and the sweep's existing cancellation-race frees the UI. (The earlier §0 "Stop now stops a hung Check All" fix only addressed `runspace.Open()`, never the `ps.Stop()` callback block.) This fix is independent of the agent.
  - **(b) Live progress — the real fix is a compiled agent, not transport-polling.** First attempt (also in the tree, then superseded) was DO/BITS transport polling in the PS worker; researched and rejected as a fragile half-measure — on **Server 2019/2022/Win10+, Delivery Optimization is the default and `Get-BitsTransfer` returns empty** (BITS runs behind DO; Bypass-to-BITS was removed in Server 2022+), so it'd depend 100% on `Get-DeliveryOptimizationStatus` (drift-prone) and still can't show **install** %. **How BatchPatch actually does it:** a compiled SYSTEM agent using WUA's own `IDownloadProgressChangedCallback`/`IInstallationProgressChangedCallback` — the ground-truth %. PowerShell can't supply those COM callbacks (the only scriptable alt is a DISPID-0 sink PS can't make — this is what the reverted `1cabb9c` Add-Type shim chased).
  - **New project `source/Vivre.UpdateAgent/` (net48).** Tiny (~23 KB) console EXE; net48 runs on every modern target with no runtime to deploy (same reason BatchPatch's agent is .NET Framework). Uses WUA COM via a **committed `Interop.WUApiLib.dll`** generated once by `tlbimp.exe` from `%SystemRoot%\System32\wuapi.dll` (`EmbedInteropTypes` → single-file EXE). **`COMReference`/`ResolveComReference` does NOT work under `dotnet`'s .NET-Core MSBuild (MSB4803) — no VS here — so the committed tlbimp output keeps `dotnet build` headless.** Files: `Program.cs` (search → whole-batch `BeginDownload`+`BeginInstall`/`BeginUninstall` with real callbacks → JSONL), `Callbacks.cs` (the 2 progress callbacks + 2 no-op completed callbacks), `ProgressWriter.cs` (thread-safe append JSONL via `System.Web.Extensions` `JavaScriptSerializer` — no NuGet), `AgentConfig.cs` (reads the JSON config). **NOT the reverted on-target Add-Type shim — this is a pre-built native EXE run standalone as SYSTEM, never compiled into a live PS session.**
  - **`WuaUpdateLane` rewired:** `BuildInstallWorker`/`BuildUninstallWorker` (the big PS here-strings) + `BuildExcludePsArray`/`BuildIncludeKbPsArray` are **gone**, replaced by `BuildAgentConfigJson(options, progressPath, mode)`. `RunWorkerTaskAsync` now base64-drops the agent EXE (`ReadAgentBytes()` from `AppContext.BaseDirectory`) + the config onto the target, registers the SYSTEM task to run **the EXE** (`-Execute '<exe>' -Argument '"<config>"'`), and **the streaming controller / heartbeat / 2-min-deadline / finally-cleanup in `BuildBootstrapScript` is UNCHANGED** — the agent writes the identical `{phase,message,percent,available,installed,failed,rebootPending,ts}` JSONL the tail already streams. Cleanup now reaps exe+config+progress. `BuildScanScript` (read-only scan over WinRM) is unchanged — scan stays PowerShell.
  - **Bundling:** `Vivre.Desktop.csproj` gets a `ReferenceOutputAssembly=false` ProjectReference (build-order) + two Copy targets (AfterBuild/AfterPublish, mirroring the PS-SDK-Modules copies) that put `Vivre.UpdateAgent.exe` beside `Vivre.exe`. Agent added to `Vivre.slnx`. `NU1702` (cross-TFM) suppressed in Desktop.
  - **Verified offline:** spike proved net48+WUA COM+callbacks compile headless and the COM objects/callbacks instantiate at runtime; full solution build 0 warnings; 95 tests green (2 old `BuildIncludeKbPsArray` tests → `BuildAgentConfigJson` tests); regenerated bootstrap parses clean under PS 5.1. (A local agent run against *online* WU search was slow/stuck on this box — environmental; the controller's heartbeat + 2-min-no-file deadline cover a genuinely slow search.)
  - **NEXT (live-verify on a real target SERVER):** drop the new `publish\Vivre-win-x64.zip`, then (1) **Stop test** — Scan a list incl. a box you reboot mid-scan, hit Stop → UI frees **instantly** (no multi-min hang); (2) **live-progress test** — Install on NYC-FP1 → download shows **real moving `X of N — P% (a/b MB)`** from WUA callbacks, then install shows live % too, then PendingReboot/Done; (3) the SYSTEM task + temp EXE/config/progress are created and **cleaned up** (incl. on a Stop mid-install). Watch for **AV/Defender** flagging an unsigned EXE in `C:\Windows\Temp` run as SYSTEM (allowlist if needed — BatchPatch hits this too). If the agent is missing beside `Vivre.exe`, install fails fast with a clear `FileNotFoundException` (bundling regressed).
