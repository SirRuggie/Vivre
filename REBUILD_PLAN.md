...
- 🧰 **Update-lane UX + concurrency + WUA-then-DISM uninstall (built 2026-05-29, working tree, 95 tests green, 0 warnings, PENDING LIVE-VERIFY).** Addressed a batch of user-reported issues:
  ...
  - **Install live-verified working on a real target (2026-05-29)** — download/install progress shown (the message % is the current step; the bar is overall download[0-50%]+install[50-100%], so "Installing 1 of 1 — 20%" reads 60% on the bar — by design). **Follow-up UX fixes (post-test, in tree):**
    ...
    - **Reboot lifecycle reporting (A2 partial):** the monitor now writes the **Reboot message** column on online↔offline transitions — `Computer.WentOfflineAt` records the down moment, and the return trip shows "Back online HH:mm (down 3m) — reboot still pending" (BatchPatch-style "it's back" signal the user was missing; previously transitions only hit the activity log). Caveat: the monitor pauses while `IsBusy`, so a reboot is caught once the operation that triggered it ends (true for the post-install reboot, since the agent writes the terminal PendingReboot line — IsBusy clears — *then* sleeps 5s and reboots).
