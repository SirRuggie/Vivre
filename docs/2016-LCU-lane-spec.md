...
4. **Stage (daytime step):** copies/downloads the package, runs the DISM install with no restart while the server keeps serving. Reboot-readiness as-built: `dism /add-package` runs **synchronously** (the servicing transaction is complete when it returns — TrustedInstaller/TiWorker quiescence is implicit), then the agent confirms via the single authoritative `CBS\RebootPending` check (present = genuinely staged; absent after a "success" exit = silent no-op, NOT marked ready). Marks "Staged — run Reboot Wave." NEVER reboots.
5. **Reboot Wave (night step):** operator selects boxes, reboots them (gracefully, escalating to forced after the go-offline window — 20 min for staged-2016 via `ForSlowCommit`, 2× that after the force). As-built, the offline commit is tracked by **raw elapsed time against fixed windows** (TCP-445 polled every 20 s; 90-min "Overdue — check console/iLO" flag that never stops the watch; 4.5-h hard cap on live tracking) — there is no learned per-box average; that was spec-time aspiration, never built.
6. **Progress UI:** real progress, not spinners. As-built phases (`PatchPhase`): the controller copies + SHA-256-verifies the package (not a phase line), then the agent emits **Staging (x%)** → **PendingReboot** ("Staged — reboot-ready"); the wave emits **Rebooting (elapsed)**, verification is a message ("Verifying build…") under the wave, and the terminals are **Done / Error** (a servicing-busy refusal is **Deferred**, never a fake success). Elapsed shows alongside percent (DISM percent stalls on 2016). ("Downloading/Installing" are WUA-lane phases; "Hash-verified"/"Verifying"/"Failed" from the original spec never existed as phases.)
7. **Post-reboot verify:** confirm UBR equals the target (9234 this cycle) → green/red per box. As-built this is UBR-ONLY (`UbrConfirmation` → `DcomLcuBuildReader` reads CurrentBuild+UBR → `FullPackageLcuLane.Decide`); the spec-time "key services running, optional role checks" were never built — no service or role check exists in the verify path (the WUA lane's confirmation checks only `LastBootUpTime`, proof of a real reboot).
