...
At diagnosis the boxes were a full cycle behind on the OS CU. DISM CheckHealth/ScanHealth clean, disk/network/time healthy, no WSUS, SentinelOne ruled out (tested disabled).
...
1. **Full-package primary lane** (`FullPackageLcuLane`, alongside `WuaUpdateLane`): for 14393 boxes, the OS LCU never installs via WUA/Express. Instead: identify the month's CU KB from WUA scan metadata → use the full package from the package folder → **expand the `.msu` to its `.cab`(s) with `expand.exe`** (online DISM on Server 2016 rejects `.msu` payloads with `0x80070032`) → install each cab **SSU-first via `dism.exe /add-package`** (the agent runs `dism.exe` and reads its `\r`-drawn progress bar in 256-char chunks — see the red-team review, trap 4). .NET and small updates stay on the existing WUA lane.
...
4. **Stage (daytime step):** copies the package and runs the DISM install with no restart while the server keeps serving. `dism /add-package` runs **synchronously** (the servicing transaction is complete when it returns — TrustedInstaller/TiWorker quiescence is implicit), then the agent confirms via the single authoritative `CBS\RebootPending` check (present = genuinely staged; absent after a "success" exit = silent no-op, NOT marked ready). Marks "Staged — run Reboot Wave." NEVER reboots.
5. **Reboot Wave (night step):** operator selects boxes, reboots them (gracefully, escalating to forced after the go-offline window — 20 min for staged-2016 via `ForSlowCommit`, 2× that after the force). The offline commit is tracked by **raw elapsed time against fixed windows**: TCP-445 polled every 20 s, a 90-min "Overdue — check console/iLO" flag that never stops the watch, and a 4.5-h hard cap on live tracking.
6. **Progress UI:** real progress, not spinners. Phases (`PatchPhase`): the controller copies + SHA-256-verifies the package (not a phase line), then the agent emits **Staging (x%)** → **PendingReboot** ("Staged — reboot-ready"); the wave emits **Rebooting (elapsed)**, verification is a message ("Verifying build…") under the wave, and the terminals are **Done / Error** (a servicing-busy refusal is **Deferred**, never a fake success). Elapsed shows alongside percent (DISM percent stalls on 2016). ("Downloading/Installing" are WUA-lane phases.)
7. **Post-reboot verify:** UBR-ONLY — confirm the UBR equals the operator's target (`UbrConfirmation` → `DcomLcuBuildReader` reads CurrentBuild+UBR → `FullPackageLcuLane.Decide`) → green/red per box. No service or role check exists in the verify path (the WUA lane's confirmation checks only `LastBootUpTime`, proof of a real reboot).
...
- The operator confirms the month's CU KB and target UBR per cycle in Settings (neither can be derived from a scan — see the backlog's Settings-simplification record).
- When a cycle's CU is a **combined SSU+LCU** there is no separate-SSU half-apply risk; otherwise the SSU installs first (the SSU-first `dism.exe /add-package` ordering above).

## Mixed-fleet "Install all" routing (settled)

One button, auto-routes per machine. **The staged-patching toggle makes the 2016 lane opt-in:** a 2016 box goes to the LCU lane (stage-and-stop) **only when the operator has flagged it** (`RequiresStagedPatching`); an unflagged 2016 box falls through to the WUA lane exactly like a 2019+ box. 2019+ → existing WUA lane (one-step). Unknown/unscanned OS → skipped with "Check Vitals first" (fail-safe — never mis-routed). When a flagged-but-not-staged 2016 box is in the target set, Install shows the **`StagedInstallDecisionDialog`** (Stage CU first / Install minor updates only / Cancel; Cancel skips only the flagged boxes); a box already at the target UBR this session (`LcuVerifiedThisSession`) skips the dialog, and the Stage step is gated on a scan-this-session. Result is a bucketed summary ("N installed, N staged — run Reboot Wave, N need reboot, N failed" — non-2016 reboot-pending boxes get their own bucket); staged 2016 rows read as action-needed, visually distinct from green-done and red-failed.
...
**History:** 2026-07-22 — the original spec's aspirational bits (Add-WindowsPackage, learned per-box commit averages, extra verify checks, spec-only phase names) were folded into the present-tense as-built text above, and per-cycle KB/UBR/package-size data points replaced with the general per-cycle rules.
