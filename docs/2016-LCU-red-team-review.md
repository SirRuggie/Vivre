...
4. **Use `Add-WindowsPackage`, not raw `dism.exe`.** dism draws its progress bar with `\r`/backspaces that turn to garbage when captured remotely; the cmdlet exposes a clean progress stream.
...
- **The 1.10.0 timeout/watchdog will false-positive.** Those per-host timeouts assume minutes; a DISM stage is 30-60 min. The lane needs phase-specific long budgets + the agent heartbeat flowing during long DISM phases, so the watchdog doesn't declare a healthy box dead.

## Late-return / UBR-decides rule (the most important wave behavior)

The clock NEVER writes a box off — only the UBR check decides pass/fail. Two separate timers: the 8-min "go offline" window (reboot escalation only) and the 90-min "offline commit" ceiling (a flag, not a stop). A box offline > 90 min that then returns is re-detected and Verified; it's never stuck "timed-out" while actually up. On return, Verify retries through "can't read registry yet" (box mid-boot) — never a false red. Verify is also a standalone button, so no box is ever stranded by a timer.
