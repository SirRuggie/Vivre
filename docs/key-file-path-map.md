...
- `source/Vivre.Core/Updates/WuaUpdateLane.cs` — the normal Windows Update lane; owns the agent bytes + `SmbAgentLane`. Install selector lives here (an install is multi-call). The scan now emits raw `MinSizeBytes`+`MaxSizeBytes` per update (both the PS scan string and the agent's `Program.cs`), not a single `SizeMb`.
- `source/Vivre.Core/Updates/UpdateSizeResolver.cs` — pure tiered display-size resolution: `ResolveDisplaySize(catalogBytes?, minBytes, maxBytes)` → WUA `MaxDownloadSize` when ≤ 10 GB (`AbsurdMaxDownloadSizeBytes`; Min when Max==0) — primary, matches BatchPatch; else the catalog size for an inflated express CU; else null (dash). `NeedsCatalogLookup(maxBytes)` gates the network call to the absurd (>10 GB) rows only. Also `ArchFromTitle`.
- `source/Vivre.Core/Updates/CatalogPageParser.cs` + `MicrosoftUpdateCatalogService.cs` (`ICatalogSizeService`) — one read-only HTTPS GET to the Microsoft Update Catalog `Search.aspx`, HtmlAgilityPack parse of the hidden `_originalSize` byte spans, arch-matched-or-largest row select; per-KB `Task` cache (one shared instance built in `App.xaml.cs`). `SelectableUpdate.DisplaySizeMb` (grid-bound, `TargetNullValue=—`) reads the resolver; `WorkspaceViewModel.ResolveCatalogSizesAsync` fills it after each scan **only for the absurd-`MaxDownloadSize` rows** (`NeedsCatalogLookup`), so normal updates incur zero catalog traffic. Display-only.
