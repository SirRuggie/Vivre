...
- `RebootWave.cs` / `IRebootWave.cs` (Vivre.Core) — the wave state machine, scoped to operator-selected + confirmed boxes only. Pre-reboot it WAITS out active servicing on a bounded settle poll (`945a4cb`; default 20 min, operator-configurable) and on expiry stops WITHOUT rebooting — it does not fail fast. Then graceful→8min→force escalation (20/40 for `ForSlowCommit`) — EXCEPT a box whose graceful DCOM reboot call is refused because a session is logged on (1191), which is forced immediately on the same DCOM call and timed on the forced window from the start (`5f6d437`). `RebootAndCommitAsync` takes a pluggable `IRebootReadinessProbe` and `IPostRebootConfirmation` so the wave is reusable across box types, and an optional `IRebootGate` for burst-rate limiting.
- `DcomRebootTrigger` (Vivre.Core/Updates) — the ONLY `Win32Shutdown` primitive site in the C# codebase (the reboot cardinal — gate grep `Win32Shutdown` → exactly one file). Used by BOTH the Reboot Wave AND `ForceRebootRunner`'s Kerberos fallback (no longer wave-only). DCOM `Win32Shutdown` (flags 2 = reboot, 6 = reboot+force), with an SMB/SCM reboot fallback (a one-shot `Vivre_Reboot_*` LocalSystem service whose image is `cmd /c shutdown /r [/f] /t 5 /c "Vivre Reboot Wave"` — i.e. **cmd.exe with `shutdown` as a child process**, not `shutdown.exe` directly — over NTLM SSO; 64337d1). **A 1191 (`ERROR_SHUTDOWN_USERS_LOGGED_ON`) refusal on a GRACEFUL call no longer takes that path** (`5f6d437`): the channel is healthy, so the trigger re-sends the FORCED form on the SAME session and returns `RebootDispatch.EscalatedToForced` (the wave then applies the FORCED go-offline window and never re-dispatches). The SMB fallback is reached only when DCOM THROWS (connect/auth/timeout — the Kerberos-broken boxes), when the escalated send itself fails or throws, or on any other non-zero/missing code — and after a 1191 on a GRACEFUL call it sends `/f` and likewise reports `EscalatedToForced`. The old unconditional-fallback behaviour was the SentinelOne "Lateral Movement" root cause — see docs/dcom-1191-reboot-fallback-findings.md. `RebootDispatch.AlreadyInProgress` (Win32 1115) = the box is already going offline on its own, never a failure.
...
- **The 1.17.x reboot-proof / verify-arc razors (all `Vivre.Core/Updates`, all pure or near-pure so `Vivre.Core.Tests` can cover them — the Desktop wiring they serve has no test home):**
  - `ShutdownReturnCode.cs` — the pure classifier over the DCOM shutdown method's raw return code → `ShutdownCallOutcome`: `0`=Accepted, `1115`=AlreadyInProgress, `1191`=GracefulRefused (`ERROR_SHUTDOWN_USERS_LOGGED_ON`), any other non-zero=Failed, and a **null `ReturnValue` is NEVER success** (`NoResultCode`). The 1115/1191 matches are EXACT, not ranges. This is the file that makes the 1191 escalation testable without a live box.
  - `UptimeRebootProof.cs` — the CLOCK-IMMUNE "did this box reboot?" test for when nobody saw it go offline. Compares UPTIME (`LocalDateTime − LastBootUpTime`, both from ONE query) rather than raw boot times, so a clock step (NTP correction on a drifted VM, manual set, DST jump) moves both and cancels out. A real reboot collapses uptime to seconds; an unreadable read returns null and is never a false success.
  - `FastRebootWatch.cs` — the timing + stand-down rules for the post-Force-reboot fast watch (1.17.1): the monitor's 20 s cadence × 2 consecutive misses gives a ~40 s detection floor, so a VM that reboots in 10–20 s never flips `IsOnline` and every downstream write hangs off that missing transition. Observational only — the reboot is unchanged, only how it is WATCHED. `MaxConcurrentBulkReads` is the slot reservation that keeps a big force-reboot selection from starving the monitor.
  - `VerifyArcTimeout.cs` — the arc's own 5-minute `Ceiling` (sized for the whole arc, deliberately NOT inherited from the probe's 120 s, which was sized for one WinRM call), `IsArcDeadline`, and `MarkUnverified` for the honest row state a cut-short arc lands on.
  - `ArcResultFreshness.cs` — guards the detached arc's verdict against overwriting a newer one: a **monotonic per-host generation counter** (not a timestamp) bumped by every non-arc `RebootRequired` writer, plus the row-claim check and `ClearProgressStamp`.
  - `ScanAttemptCap.cs` — who gets a per-attempt ceiling on the post-reboot rescan (the monitor's arc) and who must not (the wave, whose staged-2016 boxes can legitimately exceed it). **Honest note in its own doc comment: currently DOCUMENTATION, not mechanism** — it equals `VerifyArcTimeout.Ceiling` and is armed later, so the ceiling always trips first.
  - `BootTimeRefresh.cs` — re-reads one row's `LastBootTime` (the grid's "Last reboot" column) at the offline→online transition, the one point covering both the wave and a one-off Force reboot. **A failed read BLANKS the cell** — deliberately the opposite of the vitals null-guard.
  - `RebootProbeAdmission.cs` — pure decision for whether the monitor may run its reboot-pending WinRM probe against a row this pass. Now admits a force-reboot-armed row on **either** tab; it used to be Patching-only, which left the Force-reboot verify arc dead exactly where Force reboot is used.
  - `ForceRebootVerifyGate.cs` — the three-condition razor that runs the shared post-reboot verify step ONCE for a hand Force reboot (the wave runs it inline instead). Its only caller sits inside `RebootProbeAdmission`'s gate.
  - `LocalHostRebootWarning.cs` — the self-target razor for the "you are about to reboot the box Vivre is running on" line. **DISCLOSURE, NOT A GUARD** — its only consumer is dialog text; it gates, blocks, filters and reorders nothing. Four call sites (Force reboot, Reboot & verify, Schedule, the install nudge); the help text covers three of them.
