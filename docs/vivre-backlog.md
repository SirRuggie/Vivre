...
> vitals gaps filled — all this session). Everything below is on
...
- **Custom / predefined columns over DCOM (the "WinRM n/a" on Kerberos-broken boxes).** Distinct from the DCOM VITALS fix (`4c88c69`, DONE) — that filled the built-in vitals fields (stopped-services, Users-online dot) over DCOM, but the grid's custom columns and the predefined "Logged-on user" column each run their OWN private WinRM one-liner, have no DCOM fallback, and keep showing "WinRM n/a" on a Kerberos-broken box. **Why it's hard / parked:** a user-defined custom column runs an ARBITRARY PowerShell one-liner, and arbitrary PS can't run over DCOM (DCOM does specific WMI queries, not general script) — so most custom columns fundamentally CAN'T fall back to DCOM. The PREDEFINED columns (known queries like "Logged-on user") might be special-cased to a DCOM/WMI equivalent, but that's a separate, maybe-feasible investigation. **Build only if** "WinRM n/a" on those columns becomes a real day-to-day annoyance — and if so, scope it to the PREDEFINED columns only (research whether each known query has a DCOM/WMI equivalent), NOT arbitrary custom ones. Note: the "Logged-on user" predefined column could reuse the same explorer-owner-over-DCOM query the vitals fix just proved works. Raised as a "wondering" during the vitals fix, not a committed item.
...
- **DCOM vitals gaps filled — stopped auto-services + logged-on user now populate on Kerberos-broken / WinRM-down boxes — DONE** (`4c88c69`, on master). **The gap:** on a box reachable only over DCOM (WinRM down / Kerberos 0x80090322), `DcomVitalsProbe` populated disk/memory/CPU/boot/reboot/OS but left `StoppedAutoServiceCount` / `StoppedAutoServices` / `UserLoggedOn` / `LoggedOnUsers` empty — so the boxes you most need to triage showed a blank stopped-services list and no logged-on-user dot. **The fix:** two read-only `root\cimv2` queries added to `DcomVitalsProbe.ReadSync`, wired into the existing `MachineVitals` ctor (no model/UI/scorer change), mirroring the WinRM path's exact definitions so DCOM results render identically. (1) Stopped services: `Win32_Service WHERE StartMode='Auto' AND State<>'Running'` — count ALL, names capped at first 15 (matches WinRM). (2) Logged-on user: `Win32_Process WHERE Name='explorer.exe'` + per-instance `GetOwner` → `UserLoggedOn` = count>0, `LoggedOnUsers` = distinct/sorted owners. **Deliberately explorer-owner, NOT `Win32_ComputerSystem.UserName`** (rejected — UserName is blank for RDP sessions; explorer-owner catches console AND RDP, matching WinRM). **`GetOwner` instance-method overload proven, not assumed** — it's the SAME `session.InvokeMethod(namespace, CimInstance, method, params, options)` overload `DcomRebootTrigger.cs:129` already uses over `DComSessionOptions`, so the channel was already battle-tested. **Independent try-blocks** (a denied `Win32_Service` leaves the user fields intact and vice-versa) + a `GetOwner` inner try (a denied owner-lookup still sets `UserLoggedOn` from the process count, so the "Users online" dot survives; only names go empty) — same documented degrade pattern as the probe's existing blocks, no empty-catch. **Scorer unchanged:** `VitalityScorer` reads neither field, so the 0–100 score does NOT shift on Kerberos-broken boxes — only the triage breakdown fills in (corrected the original hypothesis that it would move the score). Thread-safe: none of the four fields is live-filtered; `ApplyVitals` already copies them on the UI thread; the fix only changes what the off-thread probe returns. Pure shaping helper `VitalsShaping` (count-all/names-15, distinct/sorted/blank-dropping owners) unit-tested. **658 tests** (+5). Cardinal clean. **Scope note:** this fixes the built-in VITALS fields over DCOM. It does NOT make CUSTOM columns work over DCOM — those run their own WinRM one-liners (see PARKED).
