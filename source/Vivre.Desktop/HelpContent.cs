    ...
    [
        ...
        new HelpTopic
        {
            ...
            [
                ...
                "Each box is rebooted gracefully (lets SQL/services flush). Windows REFUSES a graceful reboot on any box that has a logged-on session — Active or merely disconnected — so Vivre completes the reboot you ordered on that box as a FORCED one immediately, and the row tells you why (\"a user is logged on, so Windows refused the graceful reboot\"). Unsaved work on such a box is lost, exactly as with Force reboot. Any other box that doesn't go down within 8 minutes (20 for a staged-2016 box, whose commit is slower) is escalated to a forced reboot the same way. Either way each box gets exactly ONE reboot per click.",
