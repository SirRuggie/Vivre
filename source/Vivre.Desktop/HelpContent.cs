    ...
    [
        ...
        new HelpTopic
        {
            ...
            [
                ...
                "In-maintenance machines also show WHY: the row reads e.g. in maintenance — \"Rebuilding - SB\" (admin_sbridges, since 2026-05-27, 54d) — the reason note, who set it, and when it entered. Hover the cell for the full text if it's cut off, and the activity log adds a digest naming every in-maintenance machine. Only machines actually in maintenance pay the small extra lookup, so the check stays as fast as before for everything else; if the extra detail can't be read, the row simply shows plain \"in maintenance\" and the summary says the reason lookup failed.",
        ...
        new HelpTopic
        {
            ...
            [
                ...
                "Machines with nobody logged on reboot gracefully — forced after 8 minutes (20 for staged 2016). If anyone is logged on, including a disconnected session, the reboot call Vivre sends is refused, so Vivre forces it immediately — unsaved work is lost.",
                ...
                "  Unverified also covers two cases on the automatic check that follows a Force reboot: it gave up after 5 minutes, or you started a Check All / Check Vitals (or another reboot) on that machine while it was still running. In the second case the older check's result is discarded rather than allowed to overwrite the newer one, and the log says so. Either way the machine is left Unverified, never green — re-check it when you're ready.",
        ...
        new HelpTopic
        {
            ...
            Tip = "Set the month's KB + build number in Settings ▸ Server 2016 cumulative update (the card holds the catalog and update-history links — copy this month's KB and build number from them), and drop the Catalog .msu in the CU package folder before staging. Shortcut: once the .msu is in the CU package folder, Settings ▸ Server 2016 cumulative update ▸ Read from package reads the KB, build number, and architecture off the file (and suggests a month label like July 2026 from the file's date) and asks you to confirm — it refuses anything that isn't a genuine Server 2016 cumulative update and never changes your values without showing you.",
        },
        new HelpTopic
        {
            ...
            Tip = "Set this month's KB + build number in Settings ▸ Server 2016 cumulative update before staging or using \"Install minor updates only\" — both need to know which update is the cumulative one. Flagging is remembered between sessions. Shortcut: once the .msu is in the CU package folder, Settings ▸ Server 2016 cumulative update ▸ Read from package reads the KB, build number, and architecture off the file (and suggests a month label like July 2026 from the file's date) and asks you to confirm — it refuses anything that isn't a genuine Server 2016 cumulative update and never changes your values without showing you.",
        },
        new HelpTopic
        {
