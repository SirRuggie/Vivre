...
public partial class WorkspaceViewModel : ObservableObject
{
    ...
    /// <summary>Reads deep OS vitals (disk / memory / CPU / uptime / stopped services / recent errors)
    /// for every row and scores each 0-100. Read-only — one click, no confirm. Heavier than ping, so
    /// each row is gated by <see cref="_vitalsThrottle"/> from inside <see cref="CheckVitalsRowAsync"/>.</summary>
    [RelayCommand(CanExecute = nameof(CanStartSweep))]
    private Task CheckVitalsAsync() => RunSweepAsync([.. Computers], CheckVitalsRowAsync);
